...
**Run 1 opens the request; every run after it comments its receipt onto the
same one.** Because `-base` is a long-lived integration branch, the request
stays open across runs, so `create` would fail "already exists" from run 2
onward — the presets fall back to a comment instead. The request then
accumulates one receipt per run, in order, and the steady state exits `0`. Once
that request is merged, the next run opens a fresh one.
...
Verbatim output of `sig run -base integration -intent add-metrics`:

```
**sigbound receipt** — landed `842f30ce54ef` onto `integration`.

- intent: `add-metrics`
- landed sha: `842f30ce54efaf3a86e98b093087baae3fbeafd5` (base was `19fc0c2d97232d1ba64ebdec6098fc11581b511f`)
- verify: passed
- agents: 1 of 1 succeeded; landed: `agent/add-metrics`
- tasks:
  - `add-metrics`: add a request counter to the http handler

sigbound 1.0.0, run started 2026-07-26T05:35:12Z.
```

A `- run:` line appears above `- intent:` when the run came from
[`sig serve`](#sig-serve), which is the only thing that assigns a run id;
`- intent:` appears only for an `-intent` run.

**Anything that did NOT land gets its own line**, and those are the reason the
artifact exists:

```
- **PARKED, AWAITING ACK** (no ack timeout — it waits for a human): `9f0c1a2b3c4d` verified GREEN but was deliberately NOT landed — matched ack-paths. Held branches: `agent/b`. Triggering paths: `cmd/sig/run.go`
- **NOT LANDED** — 1 branch(es) flagged as conflicts and withheld: `agent/c` (paths: `pkg/x.go`)
- **NOT LANDED** — 1 branch(es) dropped by -verify-bisect (their group broke the combined tree): `agent/d`
```

A run can land its clean groups and [park](#run-parking) one: the base
advanced, so `-publish` fires, and the receipt is then the document in front of
the very person whose ack that group is waiting on. A receipt that reported
"2 of 2 agents succeeded" and stopped would be describing a run that is still
holding work back.
...
**Nothing in a receipt is unbounded.** Every list — tasks, landed branches,
held branches, flagged branches, triggering paths — is cut to 20 entries with
`… and N more`, and every entry (a prompt, a task id, a branch name, a path) to
its first line and 200 runes. All of it is text an LLM chose, and the result
becomes one environment variable (Linux caps a single one at 128 KiB) and one
request body (GitHub caps one at 65,536 characters); the worst case here is
roughly 25k characters.
...
```sh
command -v gh >/dev/null 2>&1 || { echo "sigbound -publish-preset=github-receipt: gh is not on PATH (install: https://cli.github.com) -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; gh auth status >/dev/null 2>&1 || { echo "sigbound -publish-preset=github-receipt: gh is not authenticated (run: gh auth login -- or, under -env-mode scoped, pass -env-publish GH_TOKEN: the scoped base env does not carry GH_TOKEN or SSH_AUTH_SOCK) -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; sb_remote=${SIGBOUND_REMOTE:-origin}; sb_target=$(git ls-remote --symref "$sb_remote" HEAD | awk '$1=="ref:" && $3=="HEAD"{sub("refs/heads/","",$2); print $2; exit}'); [ -n "$sb_target" ] || { echo "sigbound -publish-preset=github-receipt: cannot read the default branch of remote $sb_remote (git ls-remote failed; set SIGBOUND_REMOTE if the remote is not named origin) -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; [ "$sb_target" != "$SIGBOUND_BASE_BRANCH" ] || { echo "sigbound -publish-preset=github-receipt: -base $SIGBOUND_BASE_BRANCH IS the default branch of remote $sb_remote, and a pull request cannot be opened from a branch onto itself -- re-run with -base on an integration branch, or drop the preset and publish with -publish 'git push $sb_remote $SIGBOUND_BASE_BRANCH' -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; sb_refs="$SIGBOUND_BASE_BRANCH"; git show-ref --verify --quiet refs/notes/sigbound && sb_refs="$sb_refs refs/notes/sigbound"; git push --atomic --porcelain "$sb_remote" $sb_refs || { echo "sigbound -publish-preset=github-receipt: atomic push of [$sb_refs] to $sb_remote failed -- NOTHING was pushed (--atomic moves every ref or none). A rejected refs/notes/sigbound usually means the token can write branches but not refs/notes/*; a rejected branch usually means it moved underneath this run, so re-run against the new head -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; { gh pr create --base "$sb_target" --head "$SIGBOUND_BASE_BRANCH" --title "sigbound: $SIGBOUND_LANDED" --body "$SIGBOUND_RECEIPT" || { echo "sigbound -publish-preset=github-receipt: could not open a new pull request (usually because one is already open for $SIGBOUND_BASE_BRANCH); posting this run's receipt on the existing one instead" >&2; gh pr comment "$SIGBOUND_BASE_BRANCH" --body "$SIGBOUND_RECEIPT"; }; }
```

`-publish-preset gitlab-receipt`:

```sh
command -v glab >/dev/null 2>&1 || { echo "sigbound -publish-preset=gitlab-receipt: glab is not on PATH (install: https://gitlab.com/gitlab-org/cli) -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; glab auth status >/dev/null 2>&1 || { echo "sigbound -publish-preset=gitlab-receipt: glab is not authenticated (run: glab auth login -- or, under -env-mode scoped, pass -env-publish GLAB_TOKEN: the scoped base env does not carry GLAB_TOKEN or SSH_AUTH_SOCK) -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; sb_remote=${SIGBOUND_REMOTE:-origin}; sb_target=$(git ls-remote --symref "$sb_remote" HEAD | awk '$1=="ref:" && $3=="HEAD"{sub("refs/heads/","",$2); print $2; exit}'); [ -n "$sb_target" ] || { echo "sigbound -publish-preset=gitlab-receipt: cannot read the default branch of remote $sb_remote (git ls-remote failed; set SIGBOUND_REMOTE if the remote is not named origin) -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; [ "$sb_target" != "$SIGBOUND_BASE_BRANCH" ] || { echo "sigbound -publish-preset=gitlab-receipt: -base $SIGBOUND_BASE_BRANCH IS the default branch of remote $sb_remote, and a merge request cannot be opened from a branch onto itself -- re-run with -base on an integration branch, or drop the preset and publish with -publish 'git push $sb_remote $SIGBOUND_BASE_BRANCH' -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; sb_refs="$SIGBOUND_BASE_BRANCH"; git show-ref --verify --quiet refs/notes/sigbound && sb_refs="$sb_refs refs/notes/sigbound"; git push --atomic --porcelain "$sb_remote" $sb_refs || { echo "sigbound -publish-preset=gitlab-receipt: atomic push of [$sb_refs] to $sb_remote failed -- NOTHING was pushed (--atomic moves every ref or none). A rejected refs/notes/sigbound usually means the token can write branches but not refs/notes/*; a rejected branch usually means it moved underneath this run, so re-run against the new head -- the landing already happened and still stands; only this receipt is missing" >&2; exit 1; }; { glab mr create --source-branch "$SIGBOUND_BASE_BRANCH" --target-branch "$sb_target" --title "sigbound: $SIGBOUND_LANDED" --description "$SIGBOUND_RECEIPT" --yes || { echo "sigbound -publish-preset=gitlab-receipt: could not open a new merge request (usually because one is already open for $SIGBOUND_BASE_BRANCH); posting this run's receipt on the existing one instead" >&2; glab mr note "$SIGBOUND_BASE_BRANCH" --message "$SIGBOUND_RECEIPT"; }; }
```
...
`publish.log`), and the run exits `6` instead of `0` — unless a
higher-precedence condition already claimed the code, e.g. `4` for a run that
landed with a branch flagged (publish is checked last; see
[Exit codes](#exit-codes)). Read `publish.output`, not stderr, when a receipt
goes red.
...
**Under [`-env-mode scoped`](#scoped-environments), pass the host token
through.** The scoped base environment does not carry `GH_TOKEN`,
`GITHUB_TOKEN`, `GLAB_TOKEN` or `SSH_AUTH_SOCK`, so a CI runner that
authenticates by token — the normal GitHub Actions shape — fails the auth
preflight unless you allowlist it:

```bash
sig run ... -publish-preset github-receipt -env-mode scoped -env-publish GH_TOKEN
```

The preflight message names `-env-publish` as well as `auth login` for exactly
this reason: an interactive login is not a fix available to a CI job.
...
**Notes don't push by default.** `git push` never sends `refs/notes/*` unless
you ask it to. Push them **in the same transaction as the branch**, never as a
second command:

```bash
git push --atomic origin main refs/notes/sigbound
```

`--atomic` means either both refs move or neither does. Pushing them separately
leaves a window where the branch has moved and its evidence has not — and
"did this run already land?" is answered by fetching the base and looking for
that run's note, so a caller recovering from an interrupted run reads *no* and
re-runs work that already landed, paying for it twice. A plain network failure
between two pushes is enough to reach that state.

The `-publish-preset` receipt presets do exactly this, and include the notes ref
only when one exists locally (`-notes` can be off). Two limits worth knowing:

- **`--atomic` needs support on both ends.** A server that lacks it fails
  loudly rather than degrading into two pushes and silently reopening the window.
- **The token has to be able to write both refs.** One that can move branches
  but not `refs/notes/*` turns a working landing into a failed publish — with
  `--atomic`, nothing is pushed at all. The presets use `--porcelain` so git's
  own per-ref status names which ref was refused, rather than leaving anyone to
  parse prose.
...
when a batch trigger is polling faster. Scheduled intents are checked BEFORE
arrivals: an intent has a deadline and a branch does not.

**But never on every tick.** "Intents first" alone is starvation with extra
steps: an intent scheduled at or under the interval — or one whose fire simply
runs longer than an interval — is due again on every tick, and a cycle that
returned as soon as it fired would then never look at an arriving branch again.
So a due tick that FOLLOWS one that fired goes to arrivals instead. Each side
waits at most two due ticks, which is the one cost worth stating: a `schedule` is
honored to within **two** `-watch-interval`s, not one. With several intents due
at once the tick goes to the one that has waited longest (least recently fired,
ties by priority then id), so a short schedule cannot own a cell's every tick.

**An intent id must be usable as a branch name.** A fire runs on
`agent/<id>-<stamp>`, so an id that is a legal filename but not a legal git ref
component (`a..b`, `...`, a leading `.` or `-`, a trailing `.` or `.lock`) can
never fire. `sig intent new` refuses one outright; a hand-written one is reported
on each tick (`watch_error`, `at: intent-id`) and fires nothing — it is never
recorded as fired, and it does not hold up the other intents.

**Due-ness is durable.** Each cell keeps `.git/sigbound/intent-fired.json`,
mapping intent id → the time it last fired and the fire branches it has produced
that still exist, written atomically and stamped the instant the run slot is
taken — before any agent runs. That ordering is what makes the guarantees hold:
...
scheduled intent fires one EXTRA time. A recorded fire in the FUTURE degrades the
same way — a stepped clock, or a record written by a machine an hour out, would
otherwise be a schedule that silently stops until that time arrives — so a stamp
more than an hour ahead of now is IGNORED (the intent reads as never fired),
reported once on the event stream (`watch_error`, `at: intent-fired-future`), and
flagged by `sig intent show` as `firedInFuture`. The next fire re-stamps it with
a real time. That direction is deliberate throughout: a fire is a policy- and
verify-gated run, so an extra one wastes a cycle, while failing closed would be a
...
retry cadence. The branch it produced is retired: no later cycle judges it under
the CYCLE's bar (the repo policy plus `-watch-verify`), which does not include
this intent's `acceptance` and would therefore land exactly the work the
acceptance rejected.

That retirement is durable, not cached. The fire writes the branch name into
`intent-fired.json` in the SAME atomic write that stamps the time, BEFORE the run
starts, and a cycle refuses to adopt any branch the record names. So it holds
across a lost or corrupt seen-set, and across a daemon killed mid-fire — neither
of which the seen-set, a cache, could survive. The seen-set entry is still
written afterwards, as a cache on top: it saves an ancestry check, and losing it
changes nothing.

Nothing is destroyed. A red fire's branch stays on disk for whoever wants to look
at it, and stays named in the record. A fire that DID land cleanly deletes its
own branch — its content is the base now — and every fire drops the names that no
longer resolve, so the record holds exactly the fire branches that still exist
rather than one line per fire forever. [`sig gc`](#sig-gc) is the only thing that
sweeps the rest: with `-delete` it removes `agent/*` branches older than
`-older-than`, except that a branch a run manifest still references is KEPT
unless `-force` is also given — which is every fire's branch, since every fire
writes a manifest naming it.
...
**Trust boundary.** An intent is read from the WORKING TREE (that is what lets
one be run before it is committed — see [above](#intents)), and a scheduled one
is fired unattended. So on a daemon started with `-watch-agent`, whoever can
write that working tree can schedule agent execution on that machine: dropping in
an untracked `intents/x.intent` with a `schedule` line is enough, with no commit,
no review and nobody choosing to run it. The converse is what makes this a
boundary rather than a hole — LANDING an `intents/*.intent` changes nothing,
because a landing moves the base ref and never touches the working tree, so a
committed intent only goes live when someone checks it out into the daemon's
tree. Treat write access to a watching daemon's working tree as execute access on
that machine.
...
| `GET /runs` | `{runs:[{id, cell, status, startedAt, finalSHA?}]}`, newest first. `finalSHA` is the commit **this run** put on the base ref — the same rule as `sig log`'s `landedSHA`, unabbreviated here. It is absent when the ref never moved (a red `-verify` still records an integrated commit it never landed), and for a run that parked *every* group and was later **acked** it is the ack's own commit, which the report predates. It does **not** follow `status`: a run that landed its clean groups and then parked a held one (see [Run parking](#run-parking)) reports that landing while sitting at `awaiting-ack` or `rejected`. Absent for a run whose `report.json` is missing or unreadable — "nothing recorded here", not proof nothing landed. |
...
Because that state is per GIT DIRECTORY and the slot is per cell, `-repos` may
not name two paths that share one — a repo and its own linked worktree. They
would share the run journal, the watch seen-set and the intent fired record while
holding separate slots, so `sig serve` refuses to start and names both paths.

### Crash recovery

Every run directory carries a tiny `status.json` phase marker —
`{status, updatedAt, pid, pidScope}` — written atomically (write-then-rename,
same pattern `-verify-cache` uses) at each transition: `queued` the instant
...
response. Any run whose recorded owner is still alive **here** is left alone by
that startup scan entirely — that covers both a run this same process is still
actively doing and a sibling `sig serve` daemon's live run sharing the same runs
directory.

"Alive here" is two checks, not one, because a bare pid names a process only
...
change if you upgrade with runs already on disk: the first `sig run` or
`sig serve` startup after the upgrade rewrites a still-live old-binary run's
marker to `interrupted` instead of leaving it alone. The cost is the same as the
Windows over-reach below — a wrong phase, never the run — and the alternative,
trusting an unscoped pid, is the exact false-alive this closes.

The residual, narrowed to a single pid space: within the same boot and namespace,
a machine that recycles pids fast enough could hand a dead run's number to an
unrelated local process before recovery runs, in which case that run is left
alone rather than recovered. Off Linux there is no `boot_id` to fold in, so the
...
whose recorded owner it cannot show to be alive, so a live daemon's runs and a
concurrent `sig run`'s are untouched. That exclusion rests on a `kill(pid, 0)` probe,
...
| **Landed** | The newest run is `done` and advanced the base ref — including a parked run a human **acked**, whose landing is recorded in `park.json` and not in the run's own report (the report was written before the ack existed, and is never back-dated). |
...
or carrying a negative number is skipped silently. Two bounds are not silent, and
not negotiable: a file **larger than 64 KiB** is refused with a line on stderr
(the record is a handful of numbers, and how much the daemon allocates is not the
agent's decision to make), and sums that would overflow — two finite `1e308`
costs, token counts past `int64` — **saturate** at the largest finite value
rather than becoming `+Inf` or a negative number, either of which would take out
...
#   my-agent "$SIGBOUND_TASK" --usage-json > "${SIGBOUND_USAGE_FILE:-/dev/null}"
```
...
| `inputTokens` / `outputTokens` / `costUsd` | **Ingested, not measured**: the sum of whatever this run's agents wrote to `SIGBOUND_USAGE_FILE`. Sigbound never sees a token or a price itself. Omitted when nothing was reported. |
| `costAgents` | How many agents actually reported — what tells a genuine zero apart from partial coverage. Omitted when nothing was reported. |
...
run in history into `{totals, cells: [{cell, repo, usage}]}` for the fleet-wide
"how much did this cost to run" story. Its totals are the **measured** fields
only — `agentsTotal`/`agentsOk`/`agentsFailed`, `integrateWallMs`,
`verifyAttempts`, `repairAttempts`, `verifyWallMs`, `totalWallMs`,
`reportBytes`, plus `runs`/`landed` as counts. `agentWallMs` and the ingested
token/cost fields are **not** summed there; the per-landed-change versions of
those live on [`GET /board`](#board-and-metrics)'s metrics, over that endpoint's
`?limit` window.
...
corrupt, wrong shape — degrades to an empty set, and what that costs is
re-examination. Precisely: losing it never lands anything that the cycle's own
bar (the repo policy plus `-watch-verify`) has not just passed, and never lands
anything judged under a bar that does not apply to it. The arrival invariant is
half of why — an already-landed branch is recognized and retired without a run,
and a branch that fell behind is refused — and the other half is that the one
kind of branch re-examination would be UNSAFE for is excluded durably instead of
here: a [scheduled fire's own branch](#recurring-intents-schedule) is named in
`intent-fired.json` before its run starts, because that work answers to its
intent's `acceptance` and not to the cycle bar.

Two things a lost seen-set does change, neither of them a landing: a branch's
[backoff](#backoff) count resets, so an excluded branch is retried, and a
landing already [parked](#run-parking) awaiting an ack can be computed and parked
a second time. Both are re-examination, and both are visible in the inbox.

Entries are pruned when their branch disappears — a decided branch that has since
been deleted can never match a head again — so the file tracks the refs that
exist rather than growing by one line per branch this daemon has ever seen.
...
| `watch_skip` | The tick found the cell busy with a manual run or an ack, so it started nothing. Two shapes: `{cell, pending, reason}` for a batch of arrivals, `{cell, intent, reason}` for a [scheduled intent](#recurring-intents-schedule) that stays due. |
...
| `watch_error` | Something a tick could not do, always `{cell, at, error}` plus what it was about. `at` is one of: `list-refs`, `classify` (+`branch`), `resolve-base` (+`base`), `intents` (the repo's `intents/` will not parse), `intent-agent` (+`intent`: due, but no `-watch-agent`), `intent-id` (+`intent`: the id cannot be a branch component), `intent-fired` (+`intent`: the fire could not be recorded, so it may repeat), `intent-fired-future` (+`intent`: the recorded last fire is ahead of now and is being ignored). |
